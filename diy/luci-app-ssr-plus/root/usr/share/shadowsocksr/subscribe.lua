...
local v2_ss = luci.sys.exec('type -t -p ss-redir sslocal') ~= "" and "xray" or "ss"
local v2_tj = luci.sys.exec('type -t -p trojan') ~= "" and "xray" or "trojan"
local log = function(...)
...
local function processData(szType, content)
	...
	elseif szType == "ss" then
		...
		result.type = v2_ss
		result.xray_protocol = (v2_ss == "xray") and "shadowsocks" or nil
	...
	result.switch_enable = switch_enable
        ...
        result.flag = ssr.get_flag(result.alias, result.server)
	return result
end
-- wget
