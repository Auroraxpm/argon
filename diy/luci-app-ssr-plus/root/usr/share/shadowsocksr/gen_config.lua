		...
		vnext = {
			{
				...
				users = {
					{
						id = server.vmess_id,
						alterId = (server.xray_protocol == "vmess" or not server.xray_protocol) and tonumber(server.alter_id) or nil,
						security = (server.xray_protocol == "vmess" or not server.xray_protocol) and server.security or nil,
						encryption = (server.xray_protocol == "vless") and server.vless_encryption or nil,
		...
		servers = {
			{
				...
				method = ((server.xray_protocol == "shadowsocks") and server.encrypt_method_ss) or nil,
				uot = (server.xray_protocol == "shadowsocks") and (server.uot == '1') or nil,
				ivCheck = (server.xray_protocol == "shadowsocks") and (server.ivCheck == '1') or nil,
			}
		}
	}
end
...
settings:handleIndex(server.xray_protocol)
local Xray = {
	log = {
	...
	-- 传出连接
	outbound = {
		protocol = server.xray_protocol,
		settings = outbound_settings,
		-- 底层传输配置
		streamSettings = (server.xray_protocol ~= "wireguard") and {
		...
		mux = (server.xray_protocol ~= "wireguard") and {
			-- mux
